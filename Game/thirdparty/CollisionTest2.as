...
package thirdparty 
{
	...
	public class CollisionTest 
	{
		...
		_returnValue = false;
		var mainRef:Main = Main.getMain();
		
		var oneGlobalOffset:Point = mainRef.globalToLocal(clip1.localToGlobal(new Point(clip1.x,clip1.y)));
		var twoGlobalOffset:Point = mainRef.globalToLocal(clip1.localToGlobal(new Point(clip1.x,clip1.y)));
		...
		_twoClipBmpData = new BitmapData(_twoRectangle.width, _twoRectangle.height, true, 0);
		_twoClipBmpData.draw(clip1, _oneOffset); _oneRectangle = clip2.getBounds(clip2);
